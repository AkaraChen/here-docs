...
- A developer creates an engine, calls `convert(bytes, { engine })`, and receives Markdown plus warnings. They close the engine themselves.
...
1. The caller creates an engine with `createEngine()`.
2. The programmable API receives file bytes, that engine, and an optional filename hint.
3. The library detects the kind of input and selects a strategy.
4. The strategy produces Markdown. Embedded raster assets and scanned PDF pages are OCR'd and inserted at their source positions.
5. The caller receives `{ markdown, warnings }` and later closes the engine.
6. The CLI is the only surface that reads a path or stdin. It creates an engine, calls `convert()`, and closes the engine.
